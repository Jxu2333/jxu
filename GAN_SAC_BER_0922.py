...
import csv
# 没有图形界面/不在 PyCharm 会话里时，改用无交互后端
if not os.environ.get("PYCHARM_HOSTED") and not os.environ.get("DISPLAY"):
    matplotlib.use("Agg")
...
class GLRTDetector:
    ...
    def estimate_clutter_subspace(self, R_q, rank_threshold=0.9):
        ...
        vals = eigenvalues[idx]; vecs = eigenvectors[:, idx]
...
class SystemConfig:
    ...
    sinr_threshold: float = 5    # 通信SINR门限（dB）
...
class UAVISACEnvironment:
    ...
    def _get_state(self):
            ...
            if self.current_slot > 0:
                avg_sinr = self.cumulative_sinr / self.current_slot
            else:
                avg_sinr = torch.zeros_like(self.cumulative_sinr)
            sinr_threshold_linear = 10 ** (self.config.sinr_threshold / 10)
            normalized_sinr = torch.clamp(avg_sinr / sinr_threshold_linear, 0.0, 2.0)
            state.extend(normalized_sinr)
    ...
    def _calculate_state_dim(self):
        ...
        dim += self.config.num_users        # 平均SINR（归一化）
        dim += 2                            # 时间进度、能耗
        return dim
    ...
    def step(self, action: Dict[str, torch.Tensor], trajectory: Optional[torch.Tensor] = None):
        ...
        scnr_thr_lin = 10 ** (self.config.scnr_threshold / 10)
        sinr_thr_lin = 10 ** (self.config.sinr_threshold / 10)

        avg_sinr_now = self.cumulative_sinr / (self.current_slot + 1)

        all_targets_done = torch.all(self.cumulative_scnr >= scnr_thr_lin)
        all_users_done = torch.all(avg_sinr_now >= sinr_thr_lin)
        early_done = bool((all_targets_done & all_users_done).item())
        ...
        done = early_done or (self.current_slot >= self.config.time_slots)
    ...
    def _construct_dd_channel_matrix(self, k_q, l_q):
        """构建延迟-多普勒域感知信道矩阵"""
        H_dd = torch.zeros((self.otfs.M * self.otfs.N, self.otfs.M * self.otfs.N), dtype=torch.cfloat, device=self.device)
    ...
    def _calculate_reward(self, sinr_values, scnr_values, energy, power_allocation, sensing_decision):
        ...
        sinr_threshold_linear = 10 ** (self.config.sinr_threshold / 10)
        for p in range(self.config.num_users):
            if sinr_values[p] >= sinr_threshold_linear:
                r_comm += 1.0
            else:
                progress = torch.clamp(sinr_values[p] / sinr_threshold_linear, max=1.0)
                r_comm += progress.item() - 0.5
        r_comm /= self.config.num_users
...
class GANSAC:
    ...
    def _log_episode_row(self, epoch: int, traj_idx: int, episode_idx: int, episode_reward, info):
        ...
        # 逐 episode 汇总
        sinr_sat_rate = (avg_sinr >= sinr_thr_lin).float().mean().item()
        ...
        for epoch in range(self.config.max_epochs):
            ...
            for traj_idx, trajectory in enumerate(trajectories):
                ...
                for t in range(self.config.time_slots):
                    ...
                    # 记录SINR满足情况
                    sinr_threshold_linear = 10 ** (self.config.sinr_threshold / 10)
                    sinr_sat = (info['sinr'] >= sinr_threshold_linear).float().mean().item()
            ...
            self.lambda_scnr = max(0.0, float(getattr(self, 'lambda_scnr', 0.0)) + self.config.lambda_scnr_lr * (self.config.scnr_target_rate - scnr_rate))
            self.env.set_lambda_scnr(self.lambda_scnr)
    ...
    def _select_action(self, state):
        ...
        if self.prev_total_dbm is not None:
            ...
            total_dbm = float(torch.clamp(torch.tensor(total_dbm), min=low, max=high).item())
                # 二次夹紧，确保不越界
        ...
        sinr_thr_lin = 10 ** (self.config.sinr_threshold / 10)
        scnr_thr_lin = 10 ** (self.config.scnr_threshold / 10)
        # average SINR so far (avoid div-by-zero)
        denom = max(1, self.env.current_slot)
        avg_sinr_now = self.env.cumulative_sinr / denom
        sinr_def = torch.clamp((sinr_thr_lin - avg_sinr_now) / max(sinr_thr_lin, 1e-6), min=0.0).mean().item()
        ...
        a0 = float(self.config.alpha_a0); a1 = float(self.config.alpha_a1); a2 = float(self.config.alpha_a2)
        alpha_raw = a0 + a1 * float(sinr_def) - a2 * float(scnr_def)
        alpha_sig = float(torch.sigmoid(torch.tensor(alpha_raw)))
        alpha_min = float(self.config.alpha_min_frac); alpha_max = float(self.config.alpha_max_frac)
...
# === (C) 感知离散部分：余下 logits 仍按你的原逻辑筛选“在范围且未完成”的目标 ===
    ...
    def _log_training_stats(self, epoch):
        ...
        if len(self.training_stats['sinr_satisfaction']) > 0:
            print(f"SINR Satisfaction: {self.training_stats['sinr_satisfaction'][-1] * 100:.1f}%")
    ...
    def evaluate(self, num_episodes=10):
        ...
        for episode in range(num_episodes):
            ...
            for p in range(self.config.num_users):
                user_sinr_satisfied.append(1.0 if avg_sinr_per_user[p] >= sinr_threshold_linear else 0.0)
            per_user_sinr_satisfaction.append(user_sinr_satisfied)
            sinr_satisfaction.append(np.mean(user_sinr_satisfied))
        ...
        for target_idx in range(num_targets):
            ...
            while current_time < time_steps - 10:
                ...
                if torch.mean(dist_to_target) < self.config.sensing_range * 0.9:
                    ...
                    sensing_time = min(max(required_steps, 8), max_available_time, 30)  # 最少8步，最多30步(一个全局硬限制，绝对不会分配超过 30 个时隙给感知。)
                    ...
                    window_size = min(7, num_smooth_points // 2 * 2 + 1)  # 确保窗口大小为奇数
                    if window_size > 1:
                        smooth_path_np_smoothed = smooth_path_np.copy()
                        for i in range(batch_size):
                            smooth_path_np_smoothed[i, :, 0] = savgol_filter(smooth_path_np[i, :, 0], window_size, 3)
                            smooth_path_np_smoothed[i, :, 1] = savgol_filter(smooth_path_np[i, :, 1], window_size, 3)
...
def main():
    ...
    print(f"SINR Satisfaction Rate (All Users): {results['sinr_satisfaction_rate'] * 100:.2f}%")
    print(f"Per-User SINR Satisfaction Rates:")
    for p in range(config.num_users):
        print(f"  User {p + 1}: {results['per_user_sinr_satisfaction'][p] * 100:.2f}%")
    print(f"SINR Satisfaction Rate: {results['sinr_satisfaction_rate'] * 100:.2f}%")
